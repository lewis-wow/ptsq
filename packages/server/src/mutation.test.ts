...
test('Should create mutation', async () => {
  ...
  expect(await mutation.call({ input: 'John', ctx: { greetingsPrefix: 'Hello' as const } })).toBe('Hello John');
  expect(await mutation.createServerSideMutation({ greetingsPrefix: 'Hello' as const }).mutate('John')).toBe(
    'Hello John'
  );
