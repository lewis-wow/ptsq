...
test('Should create URL scalar', () => {
  ...
  expect(URLScalar.output.parse(new URL('https://example.com/'))).toBe('https://example.com/');
  ...
  expect(urlScalarInputParse.success ? null : urlScalarInputParse.error).toStrictEqual(
...
test('Should create Date scalar', () => {
  ...
  expect(DateScalar.input.parse(dateMock.toISOString())).toStrictEqual(dateMock);
  ...
  const dateScalarInputParseMock = z.string().datetime().safeParse('not_valid_url');
  const dateScalarInputParse = DateScalar.input.safeParse('not_valid_url');

  expect(dateScalarInputParse.success ? null : dateScalarInputParse.error).toStrictEqual(
