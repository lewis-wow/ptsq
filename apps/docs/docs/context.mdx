...
```ts title="ptsq server with context"
...
```ts title="ptsq server with async context"
...
```ts title="ptsq server with database connection"
...
```ts title="Middleware mutating the context"
...
```ts title="Middleware for authentication"
...
```ts title="accessing context in query"
authedResolver
  .output(z.string())
  .query(({ ctx /* user: User */ }) => ctx.user.name);
```

```ts title="accessing context in mutation"
authedResolver
  .output(PostSchema)
