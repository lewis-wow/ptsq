...
The `BaseRouter` is Typescript type, that means the client doesn't know anything about the server in runtime.
...
That means you can disable the type hinting and call wrong route.
If wrong route is called, then the server response with `BAD_REQUEST` error.
