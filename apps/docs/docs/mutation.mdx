...
The `resolve` function gets parameter `{ input, ctx }`, where the input is the actual `input` for that mutation, and the `ctx` is a context, it can be the root context or some other context if the resolver that creates the `mutation` use some middleware.
...
On the client side, the `mutation` requires to call a `mutate` method on the route.
