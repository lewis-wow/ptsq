...
resolver.args({ name: z.string() }).query({
...
GraphQL in its complexity and code duplication and tRPC in lying types in some cases. strict project structure and no support for type-safe open API.
