...
On the client it tells you that the incomming type is `MyClass`, but that is not true, incomming type will be `{ x: number; y: number }`, becase classes are not serializable.

ptsq has another solution for this and that is (scalars)[/scalars].

Scalar types do not allow you to use any non-serializable type for resolver input and output.
