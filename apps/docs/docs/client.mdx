...
But you cannot call that `noop` function, because Typescript cast that type of the `Proxy` client into something like `PsqlClient<BaseRouter>`.
The `noop` function is there for only that the `Proxy` of the function can have `apply` handler.
