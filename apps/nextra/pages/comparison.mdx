...
As you can see, we duplicated the input arguments schema in the validation Schema.
But GraphQL requires us to define the schema of args, so we cannot infer the GraphQL schema from the validation schema, because GraphQL schema types can be complex and must have a name, such as `type: User` that we returning from the resolver.
...
Both, input and output, are validated on the type level and in runtime.
