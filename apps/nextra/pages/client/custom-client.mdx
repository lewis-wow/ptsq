...
You can also create your custom client adapter.
...
As you can see you have to install `@ptsq/server` package, but only as `devDependency`, `@ptsq/client` must be as `dependency`.
...
For example, this simple client does not allow mutations, it's not very practical, it's for simplicity.
