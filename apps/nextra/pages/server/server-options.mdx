...
```ts copy filename="root-option.ts"
PtsqServer.init({
  //...,
  root: '/api',
});
...
```ts copy filename="endpoint-option.ts"
PtsqServer.init({
  //...,
...
The fetchAPI option is used to define the Request and Response objects within your ptsq application. It allows you to specify how requests are made to the server and how responses are handled.

By configuring the fetchAPI option, you can customize the underlying mechanism for sending requests and processing responses according to your specific needs or the requirements of your server environment. This flexibility ensures that the ptsq application seamlessly integrates with various backend infrastructures and communication protocols, providing a versatile solution for building efficient and reliable applications.
