...
If input validation fails, the `PtsqError` throws a `BAD_REQUEST_400` error, providing details about the validation error within the info property as a Typebox validation schema error result.

Similarly, if output validation fails, the `PtsqError` throws an `INTERNAL_SERVER_ERROR_500` error, again including information about the validation error within the info property.
